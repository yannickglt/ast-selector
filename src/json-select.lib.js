...
(function (exports) {
  ...
  var operators = {
    ...
    '<=': [
      ...
      function (lhs, rhs) {
        return (
          (is(lhs, 'number') && is(rhs, 'number') && lhs <= rhs) ||
          (is(lhs, 'string') && is(rhs, 'string') && lhs <= rhs)
        );
      },
    ],
    '>=': [
    ...
    '$=': [
      ...
      function (lhs, rhs) {
        return (
          is(lhs, 'string') &&
          is(rhs, 'string') &&
          lhs.lastIndexOf(rhs) === lhs.length - rhs.length
        );
      },
    ],
    '^=': [
    ...
    '*=': [
      ...
      function (lhs, rhs) {
        return (
          is(lhs, 'string') && is(rhs, 'string') && lhs.indexOf(rhs) !== -1
        );
      },
    ],
    '>': [
  ...
  function exprParse2(str, off) {
    ...
    if (
      typeof rhs !== 'object' ||
      rhs[0] === '(' ||
      operators[op[1]][0] < operators[rhs[1]][0]
    ) {
      v = [lhs, op[1], rhs];
    } else {
      v = rhs;
      while (
        typeof rhs[0] === 'object' &&
        rhs[0][0] != '(' &&
        operators[op[1]][0] >= operators[rhs[0][1]][0]
      ) {
        rhs = rhs[0];
      }
    ...
    for (var i = 0; i < sel.length; i++) {
      if (sel[i] === '~') {
        ...
        if (i > 1) {
          ...
          var z = {};
          for (var k in sel[at])
            if (sel[at].hasOwnProperty(k)) z[k] = sel[at][k];
          if (!z.has) z.has = [];
